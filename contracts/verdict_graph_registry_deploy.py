...
class VerdictGraphRegistry(gl.Contract):
    ...
    def bind_adjudicator(self, adjudicator_address: str) -> None:
        if gl.message.sender_address != self.owner:
            _fail('Only the registry owner can bind the adjudicator')
        if self.adjudicator_address != ZERO_ADDRESS:
            _fail('Adjudicator is already bound')
        adjudicator = Address(adjudicator_address)
        if adjudicator == ZERO_ADDRESS:
            _fail('Adjudicator cannot be the zero address')
        try:
            bound_registry = VerdictGraphAdjudicator(adjudicator).view().registry_address()
        except Exception:
            _fail('Adjudicator registry binding could not be verified')
        if bound_registry != gl.message.contract_address:
            _fail('Adjudicator registry binding does not match this contract')
        self.adjudicator_address = adjudicator

    @gl.public.write
    def bind_vault(self, vault_address: str) -> None:
        if gl.message.sender_address != self.owner:
            _fail('Only the registry owner can bind the vault')
        if self.vault_address != ZERO_ADDRESS:
            _fail('Vault is already bound')
        if self.adjudicator_address == ZERO_ADDRESS:
            _fail('Adjudicator must be bound first')
        ...
        if registry_core != gl.message.contract_address:
            _fail('Vault registry binding does not match this contract')
        if adjudicator_core != self.adjudicator_address:
            _fail('Vault adjudicator binding does not match the bound adjudicator')
    ...
    def repair_handoff_delivery(self, case_id: u256, delivery_uri: str, delivery_sha256: str) -> u256:
        if case_id not in self.case_context_json:
            _fail('Unknown case')
        if self.adjudicator_address == ZERO_ADDRESS:
            _fail('Adjudicator is not bound')
        context = json.loads(self.case_context_json[case_id])
        handoff_id = u256(int(context['handoff_id']))
        self._require_handoff(handoff_id)
        handoff = self.handoffs[handoff_id]
        if gl.message.sender_address != handoff.provider:
            _fail('Only the handoff provider can repair delivery')
        try:
            repair = json.loads(VerdictGraphAdjudicator(self.adjudicator_address).view().get_case_repair_snapshot(case_id))
        except Exception:
            _fail('Adjudicator repair state is unavailable')
        if repair.get('status') != CASE_REPAIR_REQUIRED:
            _fail('Delivery repair requires a repairable case state')
        if bool(repair.get('settlement_queued')):
            _fail('Handoff is not repairable')
        if int(_now()) > int(repair.get('recovery_deadline', 0)):
            _fail('Case recovery deadline has expired')
        if not str(repair.get('failure_code', '')).startswith('DELIVERY_'):
    ...
    def open_case(self, workflow_id: u256, handoff_id: u256, claim: str) -> u256:
        ...
        if gl.message.sender_address not in (workflow.owner, handoff.requester, handoff.provider):
            _fail('Only the workflow owner or handoff participants can open a case')
        if handoff_id in self.handoff_case_id:
            _fail('Handoff already has a dispute case')
        if self.vault_address != ZERO_ADDRESS and int(VerdictGraphVault(self.vault_address).view().handoff_status(handoff_id)) != 3:
            _fail('Vault escrow must be active before opening a case')
        policy = self.policies[workflow.policy_id]
        now = _now()
        recovery_deadline = u256(int(now) + int(policy.review_recovery_seconds))
        if int(recovery_deadline) > int(handoff.recovery_deadline):
            _fail('Handoff does not have enough remaining recovery horizon')
        case_id = self.next_case_id
        self.next_case_id = u256(int(case_id) + 1)
        bounded_claim = _bounded_text(claim, 'Case claim', MAX_TEXT_CHARS)
    ...
    def retry_case_initialization(self, case_id: u256) -> None:
        ...
        if gl.message.sender_address not in allowed:
            _fail('Only a case participant can retry initialization')
        if int(_now()) > int(context['recovery_deadline']):
            _fail('Case recovery deadline has expired')
        VerdictGraphAdjudicator(self.adjudicator_address).emit().initialize_case(case_id)
    ...
    def sync_disputed_handoff_vault_status(self, handoff_id: u256) -> u256:
        ...
        handoff = self.handoffs[handoff_id]
        self._require_handoff_participant(handoff)
        if handoff_id not in self.handoff_case_id:
            _fail('Handoff has no dispute case')
        if self.vault_address == ZERO_ADDRESS:
            _fail('Vault is not bound')
        vault_status = VerdictGraphVault(self.vault_address).view().handoff_status(handoff_id)
        if int(vault_status) not in (4, 5):
    ...
    def is_policy_issuer(self, policy_id: u256, issuer: Address) -> bool:
        return _policy_key(policy_id, 'issuer:' + _address_key(issuer)) in self.policy_issuers

    @gl.public.view
    def is_policy_publisher(self, policy_id: u256, publisher_prefix: str) -> bool:
        return _policy_key(policy_id, 'publisher:' + publisher_prefix) in self.policy_publishers
    ...
    @gl.public.view
    def get_adjudicator_address(self) -> str:
        return self.adjudicator_address.as_hex
