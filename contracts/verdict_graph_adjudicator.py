...
class VerdictGraphAdjudicator(gl.Contract):
    ...
    def _require_revision(self, case_id: u256, revision_no: u256) -> None:
        if self._revision_storage_key(case_id, revision_no) not in self.revisions: _fail("Unknown case revision")
    def _context(self, case_id: u256) -> dict:
        if case_id not in self.case_context_json: _fail("Unknown case context")
        try: value=json.loads(self.case_context_json[case_id])
        except Exception: _fail("Stored case context is invalid")
        if not isinstance(value,dict): _fail("Stored case context is invalid")
        return value
    def _require_participant(self, context: dict) -> None:
        if gl.message.sender_address not in (Address(str(context["requester"])),Address(str(context["provider"]))): _fail("Only a handoff participant can perform this action")
    ...
    def bind_vault(self, vault_address: str) -> None:
        if gl.message.sender_address != self.owner: _fail("Only the adjudicator owner can bind the vault")
        ...
        if registry_core != self.registry_address_value: _fail("Vault registry binding does not match this adjudicator")
        if adjudicator_core != gl.message.contract_address: _fail("Vault adjudicator binding does not match this contract")
        self.vault_address=vault

    @gl.public.write
    def initialize_case(self, case_id: u256) -> None:
        if gl.message.sender_address != self.registry_address_value: _fail("Only the bound registry can initialize cases")
        if case_id in self.cases: return
        raw=VerdictGraphRegistry(self.registry_address_value).view().get_case_context(case_id)
        try: context=json.loads(raw)
        except Exception: _fail("Registry case context is invalid")
        if not isinstance(context,dict) or int(context.get("case_id",0)) != int(case_id): _fail("Registry case context does not match case id")
        if int(context.get("recovery_deadline",0)) <= int(_now()): _fail("Case context recovery deadline is invalid")
        opener=Address(str(context["opener"])); revision_no=u256(1); self.case_context_json[case_id]=_canonical_json(context)
        ...
        self.latest_case_by_opener[opener]=case_id; self.case_count=u256(int(self.case_count)+1)

    @gl.public.view
    def get_case_repair_snapshot(self, case_id: u256) -> str:
        self._require_case(case_id); case=self.cases[case_id]; revision=self.revisions[self._revision_storage_key(case_id,case.current_revision)]
    ...
    def resolve_case(self, case_id: u256) -> u256:
        ...
        for evidence_id_int, record in evidence_records:
            ...
            if int(now) - int(record.observed_at) > int(context["max_evidence_age_seconds"]):
                ...
                < int(context["minimum_remaining_validity_seconds"])
            ):
        ...
        def evaluate_once() -> dict:
            ...
            if str(delivery.get("uri", "")) != "":
                try:
                    delivery_response = gl.nondet.web.request(
                        str(delivery.get("uri", "")), method="GET"
                    )
                except Exception:
                    return {
            ...
            source_set_payload = {
                "policy_fingerprint_sha256": str(context["policy_fingerprint_sha256"]),
                "delivery_version": int(u256(int(delivery.get("version", 0)))),
                "delivery_sha256": delivery_observed_sha256,
                "delivery_submitted_at": int(u256(int(delivery.get("submitted_at", 0)))),
                "handoff_deadline": int(u256(int(context["handoff_deadline"]))),
                "evidence": source_set,
                "response_author": (
...
"""
    ...
    def recover_case(self, case_id: u256) -> None:
        ...
        if self.vault_address != ZERO_ADDRESS:
            ...
            VerdictGraphVault(self.vault_address).emit().apply_final_verdict(
                ...
                case.workflow_id,
                case.handoff_id,
                str(self._context(case_id)["policy_fingerprint_sha256"]),
    ...
    @gl.public.view
    def get_vault_address(self) -> str: return self.vault_address.as_hex
