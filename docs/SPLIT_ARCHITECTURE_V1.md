...
Both ICs independently verify both immutable Vault controller getters before binding. External EVM effects are explicitly emitted with `on="finalized"`; Registry and Adjudicator never rely on accepted-stage EVM effects.
...
5. Bind Vault in Registry and Adjudicator; each verifies both immutable controller addresses.
